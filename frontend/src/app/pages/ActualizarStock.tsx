...
import { actualizarStockSchema, motivoAjusteSchema, type ActualizarStockFormData, type MotivoAjusteFormData } from '../lib/validations';
...
export default function ActualizarStock() {
  ...
  const handleActualChange = (id: number, value: string) => {
    // Validación con Zod
    const validationResult = actualizarStockSchema.safeParse({ actual: value });
  ...
  const handleAjusteChange = (id: number, value: string) => {
    // Validación básica - debe ser un número
    const numValue = parseInt(value, 10);
    if (isNaN(numValue)) {
      toast.error('El ajuste debe ser un número válido');
      return;
    }
  ...
  const handleGuardar = async () => {
    ...
    // Validación del motivo con Zod
    const validationResult = motivoAjusteSchema.safeParse({ motivo: motivoAjuste });
    if (!validationResult.success) {
      const errors = validationResult.error.errors;
      const firstError = errors[0];
      toast.error(firstError.message);
    ...
    try {
      const results = await Promise.all(cambios.map(async (prod) => {
        ...
        if (nuevoStock < 0) {
          toast.error(`El stock no puede ser negativo para el producto ${prod.codigo}`);
          return { ok: false };
        }
